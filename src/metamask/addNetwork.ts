...
export const addNetwork = (page: Page, version?: string) => async ({
  ...
  const networkButton = await page.waitForSelector('.network__add-network-button > button')
  ...
  const rpcInput = await page.waitForSelector('.networks-tab__add-network-form-body > div:nth-child(2) > label > input')
